<div class="listPage">
    <div id="shorter" class="clearfix listPage">
        <div id="left_menu">
            <ul>
                <li><a href="#">Highlights</a></li>
                <li><a href="#">Articles</a></li>
                <li><a href="#">Contest</a></li>
            </ul>
        </div><!-- END #left_menu -->
        <div id="right_menu">
            <form method="post" action="" class="searchTop" id="searchTop">
				<input type="text" value="Search..."  onBlur="if(this.value=='')this.value='Search...';" onFocus="if(this.value=='Search...')this.value='';"/>
                <input type="submit" value="&nbsp;" id="querySubmit">					               
            </form>
            <ul>
                <li>
                    <div class="toggle">
                        <div class="title">
                        <a href="#" class="toggle">Category</a>
                        </div>
                        <div class="inner">
                            <a href="#">Painting</a>
                            <a href="#">Calligraphy</a>
                            <a href="#">Wall Painting</a>
                            <a href="#">Wood</a>
                        </div>
                    </div>
                </li>
                <li>
                    <div class="toggle">
                        <div class="title">
                        <a href="#" class="toggle">Newest</a>
                        </div>
                        <div class="inner">
                            <a href="#">Dayly</a>
                            <a href="#">Weekly</a>
                            <a href="#">Monthly</a>
                        </div>
                    </div>
                </li>
            </ul>
        </div><!-- END #right_menu -->
    </div><!-- END #shorter -->
    <div class="section">
        <ul class="columns-content page-content clearfix">
            <li class="col-sidebar">
               <?php include('widget/tag.php'); ?>
               <?php include('widget/weekly_popular.php'); ?>
               <?php include('widget/side_banner.php'); ?>
            </li> <!-- END .col-sidebar -->
            <li class="col-main listPage">
                <div id="right_menuR">
                    <form method="post" action="" class="searchTop" id="searchTop">
						<input type="text" value="Search..."  onBlur="if(this.value=='')this.value='Search...';" onFocus="if(this.value=='Search...')this.value='';"/>
                        <input type="submit" value="&nbsp;" id="querySubmit">					               
                    </form>
                    <ul>
                        <li>
                            <div class="toggle">
                                <div class="title">
                                <a href="#" class="toggle">Category</a>
                                </div>
                                <div class="inner">
                                    <a href="#">Painting</a>
                                    <a href="#">Calligraphy</a>
                                    <a href="#">Wall Painting</a>
                                    <a href="#">Wood</a>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div class="toggle">
                                <div class="title">
                                <a href="#" class="toggle">Newest</a>
                                </div>
                                <div class="inner">
                                    <a href="#">Dayly</a>
                                    <a href="#">Weekly</a>
                                    <a href="#">Monthly</a>
                                </div>
                            </div>
                        </li>
                    </ul>
                </div><!-- END #right_menuR -->
                <div id="articlePage">
                    <div class="blog-content clearfix">
                        <div class="article_boxBig">
                            <a href="#popup-photography" class="showPopup"><img src="content/thumb/img_content8.jpg" /></a>
                            <div class="pita_featured"></div>
                            <div class="entry-box">  
                                <div class="judul_photo">
                                    <h3>Lorem ipsum dolor sit ame</h3>
                                    <span>8 January 2013</span>
                                </div><!--END .judul_photo-->
                                <div class="content_action right">
                                    <ul>
                                        <li><a class="icon_love" href="#">55</a></li>
                                        <li><a class="icon_comment" href="#">35</a></li>
                                        <li><a class="icon_view" href="#">60</a></li>
                                    </ul>
                                </div><!--END .content_action-->
                             </div>
                        </div><!--END .article_boxBig-->
                        <div id="photo_gallery">
                            <div class="box photo cols2">
                                <a href="#popup-photography" class="showPopup thumb300">
                                    <img class="greyscale" src="content/thumb/img_content9.jpg"/>
                                </a>
                                <div class="entry-box">  
                                    <div class="judul_galery">
                                        <h3>Hans</h3>
                                        <span>Urban</span>
                                    </div>
                                    <div class="content_action right">
                                        <ul>
                                            <li><a class="icon_love" href="#">55</a></li>
                                            <li><a class="icon_comment" href="#">35</a></li>
                                            <li><a class="icon_view" href="#">60</a></li>
                                        </ul>
                                    </div><!--END .content_action-->
                                </div>
                            </div><!--END .box-->
                            <div class="box photo cols2">
                                <a href="#popup-photography" class="showPopup thumb300">
                                    <img class="greyscale" src="content/thumb/img_content10.jpg"/>
                                </a>
                                <div class="entry-box">  
                                    <div class="judul_galery">
                                        <h3>Mui</h3>
                                        <span>Potrait</span>
                                    </div>
                                    <div class="content_action right">
                                        <ul>
                                            <li><a class="icon_love" href="#">55</a></li>
                                            <li><a class="icon_comment" href="#">35</a></li>
                                            <li><a class="icon_view" href="#">60</a></li>
                                        </ul>
                                    </div><!--END .content_action-->
                                </div>
                            </div><!--END .box-->
                            <div class="box photo cols2">
                                <a href="#popup-photography" class="showPopup thumb300">
                                    <img class="greyscale" src="content/thumb/img_content11.jpg"/>
                                </a>
                                <div class="entry-box">  
                                    <div class="judul_galery">
                                        <h3>Mathias</h3>
                                        <span>Wall</span>
                                    </div>
                                    <div class="content_action right">
                                        <ul>
                                            <li><a class="icon_love" href="#">55</a></li>
                                            <li><a class="icon_comment" href="#">35</a></li>
                                            <li><a class="icon_view" href="#">60</a></li>
                                        </ul>
                                    </div><!--END .content_action-->
                                </div>
                            </div><!--END .box-->
                            <div class="box photo cols2">
                                <a href="#popup-photography" class="showPopup thumb300">
                                  <img class="greyscale" src="content/thumb/img_content12.jpg"/>
                                </a>
                                <div class="entry-box">  
                                    <div class="judul_galery">
                                        <h3>Toby</h3>
                                        <span>Gress</span>
                                    </div>
                                    <div class="content_action right">
                                        <ul>
                                            <li><a class="icon_love" href="#">55</a></li>
                                            <li><a class="icon_comment" href="#">35</a></li>
                                            <li><a class="icon_view" href="#">60</a></li>
                                        </ul>
                                    </div><!--END .content_action-->
                                </div>
                            </div><!--END .box-->
                            <div class="box photo cols2">
                                <a href="#popup-photography" class="showPopup thumb300">
                                    <img class="greyscale" src="content/thumb/img_content13.jpg"/>
                                </a>
                                <div class="entry-box">  
                                    <div class="judul_galery">
                                        <h3>Shens</h3>
                                        <span>Line</span>
                                    </div>
                                    <div class="content_action right">
                                        <ul>
                                            <li><a class="icon_love" href="#">55</a></li>
                                            <li><a class="icon_comment" href="#">35</a></li>
                                            <li><a class="icon_view" href="#">60</a></li>
                                        </ul>
                                    </div><!--END .content_action-->
                                </div>
                            </div><!--END .box-->
                        </div><!-- END #photo_gallery-->
                    </div> <!-- END .blog-content -->
                     <div class="paging">
                         <p class="clearfix">
                            <a href="#" class="prev">&nbsp;</a>
                            <a href="#">1</a>
                            <a href="#" class="current">2</a>
                            <a href="#">3</a>
                            <a href="#">4</a>
                            <a href="#">5</a>
                            <a href="#">6</a>
                            <a href="#">7</a>
                            <a href="#">8</a>
                            <a href="#">9</a>
                            <a href="#">10</a>
                            <a href="#" class="next">&nbsp;</a>
                         </p>
                    </div> <!-- END .paging -->
                </div><!-- END #articlePage -->
            </li><!-- END .col-main -->    
        </ul><!-- END .page-content -->
    </div><!-- END .section -->
</div><!-- END .listPage -->
<?php include('widget/popup_photography.php'); ?>