	...
	function main(){
		...
		if($CONFIG['MAINTENANCE']==true){
			...
			$this->mainLayout(TEMPLATE_DOMAIN_WEB . '/master.html');
				...
				$this->assign('timeoutmop', $CONFIG['MOP_TIME_TRACK']);				
				...
				if($this->isUserOnline()) {
					...
					$promoreferenceHelper = $this->useHelper('promoreferenceHelper');
					$res = $promoreferenceHelper->sendDataPromoLogAfterLoginOnly();
		...
		if($this->isUserOnline()) {
			...
			if($users){
				if($users->n_status==0&&!in_array($page,$CONFIG['access-unverified'])) {
					...
					// di comment (berlaku utk semua user)
