...
class merchandiseStock extends Admin{
	var $category;
	var $type;
	...
	function home(){
		...
		$filter .= $startdate=='' ? "" : "AND con.postdate >= '{$startdate}' ";
		$filter .= $enddate=='' ? "" : "AND con.postdate < '{$enddate}' ";		
		$filter .= $search=='' ? "" : "AND (con.name LIKE '%{$search}%' OR con.detail LIKE '%{$search}%') ";
		...
		$sql = "SELECT count(*) total FROM {$this->dbclass}_merchandise con WHERE n_status<>3 {$filter}";
		...
		$sql = "
			SELECT con.*
			FROM {$this->dbclass}_merchandise con
			WHERE con.n_status<>3 {$filter}
			ORDER BY con.postdate DESC
	...
	function add(){	
		...
		$data['detail'] 		= $this->_p('detail');
		$data['stock'] 			= $this->_p('stock');
		$data['prize'] 		= $this->_p('prize');		
		$data['postdate'] 	= $this->_p('postdate');
		...
		if($this->_p('simpan')){		
			...
			$sql = "INSERT INTO {$this->dbclass}_merchandise (name,detail,stock,prize,postdate,n_status) 
			VALUES ('{$name}','{$detail}','{$stock}','{$prize}','{$postdate}','{$n_status}')";
			$this->query($sql);
			// pr($sql);
	...
	function edit(){
		...
		if(! $this->_p('simpan')){
			...
			$sql = "SELECT * FROM {$this->dbclass}_merchandise WHERE id={$id} LIMIT 1";
			...
			$id 			= $this->_p('id');
			$name 			= $this->_p('name');
			$detail 		= $this->_p('detail');
			$stock 			= $this->_p('stock');
			$prize 		= $this->_p('prize');
			$postdate 		= $this->_p('postdate');
			...
			$sql = "UPDATE {$this->dbclass}_merchandise SET name='{$name}',
															detail='{$detail}',
															stock='{$stock}',
															prize='{$prize}',
															postdate='{$postdate}',
	...
	function hapus(){
		$id = $this->_g('id');
		if( !$this->query("UPDATE {$this->dbclass}_merchandise SET n_status=3 WHERE id={$id}")){
	...
	function inputImage($id,$img){
		$this->query("UPDATE {$this->dbclass}_merchandise SET image='{$img}' WHERE id={$id}");
	}
	
	function inputImageThumb($id,$img){
		$this->query("UPDATE {$this->dbclass}_merchandise SET thumbnail_image='{$img}' WHERE id={$id} ");
	}
