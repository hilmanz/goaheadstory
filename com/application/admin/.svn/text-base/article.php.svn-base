...
class article extends Admin{
	...
	function home(){
		...
		$filter .= $search=='' ? "" : "AND (con.title LIKE '%{$search}%' OR con.brief LIKE '%{$search}%' OR con.content LIKE '%{$search}%'  OR con.tags LIKE '%{$search}%') ";
		...
		$list_totalComment = $this->fetch("SELECT contentid,COUNT(contentid) AS total_comment FROM athreesix_news_content_comment GROUP BY contentid",1);	
		...
		$sql = "
			...
			LEFT JOIN (SELECT contentid,COUNT(contentid) AS total_comment FROM athreesix_news_content_comment GROUP BY contentid) tot ON con.id = tot.contentid
		...
		if ($list) {
			foreach($list as $key => $val){
				$id=$val['id'];
				$val['no'] = $n++;
				//$val['total_comment'] = $arrtotalcomment[$id];
				$list[$key] = $val;
			}
		}
		$this->View->assign('list',$list);
		$this->Paging = new Paginate();	
		$this->View->assign("paging",$this->Paging->getAdminPaging($start, $this->total_per_page, $total, "?s={$this->folder}&article_type={$article_type}&id_cat={$id_cat}&startdate={$startdate}&enddate={$enddate}&search={$search}"));
	...
	function totalComment(){
		$id = $this->_g('id');
		$sql = "SELECT COUNT(*) AS total_comment FROM athreesix_news_content_comment WHERE id={$id};";
		$type = $this->fetch($sql);
		return $type;
	}
	...
	function add(){	
		...
		if($this->_p('simpan')){
			...
			$sql = "INSERT INTO {$this->dbclass}_news_content (title,brief,content,articleType,categoryid,url,sourceurl,n_status,created_date,posted_date,expired_date,tags,topcontent,authorid,fromwho) 
			VALUES ('{$title}','{$brief}',\"{$content}\",'{$articleType}','{$id_cat}','{$url}','{$sourceurl}','{$n_status}',NOW(),'{$posted_date}','{$expired_date}','{$tags}','{$topcontent}','{$authorid}','{$this->fromwho}')";
			$this->query($sql);
			 //pr($sql);
	...
	function getCategory(){
		$arrPage = array(1=>"6",3=>"2",15=>"10",4=>"3",5=>"5",6=>"4",24=>"");
		if (intval($this->_p('idtype'))) {
			$pageid = $arrPage[intval($this->_p('idtype'))];
		} else {
			if ($this->_g('article_type')) {
				$pageid = $arrPage[intval($this->_g('article_type'))];
			} else return false;
		}
		
		$sql_type ="SELECT id,category,pageid FROM {$this->dbclass}_news_content_category WHERE pageid IN ({$pageid}) ORDER BY id";
		$qData = $this->fetch($sql_type,1);
		
		if(!$qData) $qData = 0;
		if ($this->_p('idtype')) {
			print json_encode($qData);
			exit;
		} else {
			return $qData;
		}
	...
	function comment($cid=null,$all=false,$start=0,$limit=5){
		global $CONFIG;
		...
		$sql = "
			SELECT c.title,cm.userid,sm.name,sm.last_name,cm.date,cm.comment
			FROM athreesix_news_content c
			LEFT JOIN athreesix_news_content_comment cm ON c.id = cm.contentid
	...
	function hapusComment(){
		$id = $this->_g('id');
		if( !$this->query("UPDATE {$this->dbclass}_news_content_comment SET n_status=3 WHERE id={$id}")){
			return $this->View->showMessage('Gagal',"index.php?s={$this->folder}");
		}else{
			return $this->View->showMessage('Berhasil',"index.php?s={$this->folder}");
		}
