...
class uploadHelper {
	...
	function copymergetextimages($images,$text){
		...
		global $CONFIG;
			...
			$image = imagecreatetruecolor(592, 444);

			$sorce = imagecreatefromjpeg($images);
			imagecopyresized($image, $sorce, 0, 0, 0, 0, 592, 444, $width,$height);
		
		   $wihte = imagecolorallocate($image, 0, 0, 0);
		   $font = "../public_html/assets/font/Candice.ttf";
		  // $font_size = ceil(($width +  $height )/ 70); 
		  $font_size = '12'; 
		   ...
		   $textwtr='';
		   $setengahwidht = ceil(592/2 - 50);
		   $y = ceil(444/2 - 100);
		   $batas=9;
		   $bbox = imageftbbox(14, 0, $font, "Action Seeker");
			$cord_x = (imagesx($image) - $bbox[4])/2; 
		   imagettftext($image, 30, 0, $cord_x-60, $y, $wihte, $font,"Action Seeker");
		   if($jumlahExplode < $batas)
				{
					...
					$cord_x = (imagesx($image) - $bbox[4])/2; 
					$y = $y ;
					imagettftext($image, $font_size, 0, 	$cord_x, $y+30, $wihte, $font,$text);
				}
				else
				{
				   foreach ( $explod as $extxt)
							...
							if($i == $batas)
								{
									...
									$x = $x+$j;
											*/
									$y = $y + 30;
									$bbox = imageftbbox(14, 0, $font, $textwtr);
									$cord_x = (imagesx($image) - $bbox[4])/2; 	
					...
					$bbox = imageftbbox(14, 0, $font, $textwtr);
					$cord_x = (imagesx($image) - $bbox[4])/2; 
					imagettftext($image, $font_size, 0, $cord_x+20, $y+30, $wihte, $font,$textwtr);
				}
		   $rand= rand(3,99);
			$date=date('ymd');
	...
	function copymergeimages($images,$imageswater){
		 
		global $CONFIG;
			
		$stamp = imagecreatefromjpeg($imageswater);
