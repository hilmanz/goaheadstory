...
class eventHelper{
	...
	function eventList($id=false, $start=null,$limit=10)
	{
		...
		$sql = "
			...
			WHERE type = 3 AND n_status IN (1,2) {$filter} ORDER BY created_date DESC,id DESC LIMIT {$start},{$limit}
		";
		// pr($sql);
	...
	function getCity($id=false, $default=false)
	{
		...
		$sql = "SELECT * FROM  {$CONFIG['DATABASE_WEB']}.city_references WHERE city NOT LIKE '%NOT SPECIFIED%' {$filter}";
	...
	function insertnewevent()
	{
		...
		$sql = "INSERT INTO {$CONFIG['DATABASE_WEB']}.events (`title`, `brief`, `content`, `type`, `created_date`, `posted_date`, `cityid`, `n_status`) 
					VALUES ('{$title}', '{$brief}', '{$content}', '{$type}', '{$created_date}', '{$posteddate}', '{$city}', '{$n_status}')";
			// pr($sql);
	...
	function storeDataEvent()
	{
		...
		$sql = "UPDATE {$CONFIG['DATABASE_WEB']}.events SET title='{$title}', brief='{$brief}', content='{$content}', type={$type}, unpublished_date='{$unpublished_date}' posted_date='{$posteddate}', cityid = {$city}, n_status = {$n_status} WHERE id = {$id} LIMIT 1";
			// pr($sql);
		$res = $this->apps->query($sql);
